 ...
 *
 * Über mehrere Arbeitsbereiche auf einmal — der Kalender kennt keinen einen.
 *
...
export async function span(
  ...
  // Ohne Bereiche keine Abfrage: `ANY('{}')` träfe nichts, liefe aber.
  if (workspaceIds.length === 0) return [];
  const alive = withDone ? ALIVE_WITH_DONE : ALIVE;
  return queryRows<TaskRow>(
    q,
    `SELECT ${COLUMNS} FROM tasks
      WHERE workspace_id = ANY($1::uuid[]) AND ${alive}
        AND COALESCE(planned_at, due_at) >= $2
