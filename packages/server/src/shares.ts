...
/**
 * Ob Freigaben gehen — also ob der Schlüssel dieser Instanz da ist.
 *
 * Der Name bleibt, die Sache liegt jetzt in `secretbox.ts`: es gibt einen
 * zweiten Aufrufer (Einladungen), und zwei Kopien derselben Verschlüsselung
 * sind zwei Stellen, an denen ein Verfahren gewechselt werden müsste.
 */
export const shareKeyPresent = keyPresent;
...
export async function listShares(
  ...
  return rows.map((r) => ({ ...r, token: unseal(r.token_enc), projectName: r.project_name }));
}
